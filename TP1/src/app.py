...
def draw_preview(image_rgb: np.ndarray, box_xyxy: np.ndarray, points) -> np.ndarray:
    """Draw bbox and points on image for preview"""
...
# --- Étape 2 : Session state initialization ---
...
img = load_image_rgb(img_path, max_dim=MAX_DIM)
H, W = img.shape[:2]
...
st.image(
    preview,
    caption="Prévisualisation : bbox + points (avant segmentation)",
